{ pkgs, ... }:
{
  ...
  # Zellij responds to SIGTERM/SIGHUP the same, by detaching and becoming a background process.
  # This is useful for cases where you close the terminal accidentally, but annoying for shutdown / restart.
  systemd.user.services."zellij-cleanup" = {
    Unit = {
      Description = "Kill zellij sessions on logout/shutdown";
      PartOf = [ "graphical-session.target" ];
    };
    Service = {
      Type = "oneshot";
      RemainAfterExit = true; # keep unit 'active' so ExecStop will run
      ExecStart = "${pkgs.coreutils}/bin/true"; # no daemon to start
  ...
  home.file.".config/zellij/config.kdl".text = ''
      keybinds clear-defaults=true {
        ...
        pane {
            ...
            bind "k" "Right" { MoveFocus "Right"; }
            bind "l" "Down" { MoveFocus "Down"; }
        ...
        tmux {
            ...
            bind "l" { MoveFocus "Up"; SwitchToMode "Normal"; }
